...
public:
	...
	bool operator== (const Impressora &impressora) const;
...
public:
   ...
   int getNumPaginasImprimir() const;
   bool imprime(Documento doc1);
   void imprimepreto(int numpag, float percentage) {
	   numPagImprimirPreto = numPagImprimirPreto - numpag*percentage;
	}
   void imprimeamarelo(int numpag, float percentage) {
	   numPagImprimirAmarelo = numPagImprimirAmarelo - numpag*percentage;
   }
   int getPrinterType() { return 2; }
};
